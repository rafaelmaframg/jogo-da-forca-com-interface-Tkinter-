...
def cadastro_palavra():
    ...
    try: 
        ...
        b2 = Button(quadro, text="NOVA", command=lambda:novapalavra()) # botão para acionar a função de adicionar palavra
        b2.place(x=250,y=10)
...
def cadastro_novapalavra(palavra):
    ...
    b5 = Button(quadro2, text="Listar Palavras", command=lambda:listar_palavras(listapalavras))# botao listar palavras inserido para dar continuidade a tela
    b5.pack()
...
def sobre():
    messagebox.showinfo(title="Jogo da Forca",message="Procurando sempre aprimorar! \nPara maiores informações ou sugestões: Rafaelmafra@live.com")
